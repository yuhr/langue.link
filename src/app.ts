...
op.initialize({
  ...
  router.get('/api/auth/interaction/:grant', async ctx => {
    ...
    if (details.interaction.error === 'login_required') {
      ...
      const { username, email, password } = ctx.session!.credentials
  ...
  router.get('/api/auth/interaction/:grant/login', async ctx => {
    ...
    const credentials = { username, email, password } as Credentials
    // verify account credentials
    const account = await Account.findBy(Credentials.check(credentials))
    const result = {
      login: {
    ...
    .then((issuer: any) => {
      ...
      router.post('/api/auth', bodyparser(), async ctx => {
        ...
        try {
          ...
          ctx.status = 303 // because of POST => GET
        } catch (err) {
